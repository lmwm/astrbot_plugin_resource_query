...
- **移除 JM 配置页的「启用下载功能」开关**：模块总开关统一由 AstrBot 原生配置 `modules.jm_enabled` 控制，关闭后模块不再加载，指令与配置页同时隐藏，重复的页内开关已无必要
- `/jm` 指令不再检查 `jm_enabled`，直接由模块是否加载决定可用性；历史配置文件中残留的 `jm_enabled` 字段会被忽略
...
- **全项目图标改为黑白单色**，不再使用彩色 Emoji：
  - 模块图标：MiMo `▤`、华数 `◎`、JM `⇩`、插件更新 `⇧`
  - 状态提示：`√` 成功、`×` 失败、`!` 警告；移除 🔍 等装饰符号
  - Pages 按钮：登录 / 测试 / 保存改为纯文字，密码切换按钮改为「显示 / 隐藏」
  - 插件 logo 由彩色转为黑白
- 对应调整密码输入框右侧留白，避免文字按钮覆盖输入内容
...
- 详情的 ID 行改为把空格放在字母中间，使全角冒号与「标题」「作者」对齐
- 消息中的漫画标题不再截断，完整显示（仍优先使用含中文的简短名）
...
- **无效缓存的漫画信息不再复用**：`info.json` 中名称为「未知」或获取失败的历史记录会被忽略并重新获取，修复详情一直显示「标题：未知」的问题
- **命名变更后的残留目录**：图片只从当前使用的 `JM<ID>-<标题>` 目录收集，并在下载完成后清理其它同前缀的旧目录，避免同一部漫画重复占用磁盘或 PDF 出现重复页

### ⚡ 变更

- 并发默认值调整：漫画级 `2`、章节级 `2`、图片级 `10`
...
- **JM 详情改为固定五行格式**（字段对齐，缺失值用占位符补齐）：
  ...
  作者缺失显示「未知」，页数缺失显示「未知」，简介为空显示「空」

- **新增「详情在下载完成后发送」开关**：
  - 关闭（默认）：下载前发送；首次下载时接口不提供页数，该行显示「未知」
  - 开启：下载完成后发送，页数按实际图片数修正，必然准确
- 使用本地缓存时同样发送详情，是否发送仅受「发送漫画信息」开关控制
...
- **JM 命名优先使用简短名**：图片目录与 PDF 文件名统一为 `JM<ID>-<标题>`，标题优先取含中文的 `oname`（原始名），为空或不含中文时回退完整标题
  - 例：`JM1210143-贝蒂喜欢恶作剧2`，不再是冗长的完整标题
- **JM 消息流程调整**：
  1. 先单独发送一条漫画信息（ID、标题、作者、页数、简介），是否发送由「发送漫画信息」开关控制
  2. 再发送 `JM<ID> 开始下载…`
  3. 下载完成后按「下载后发送 PDF」开关决定是否发送文件
- **消息标题精简**：消息中的标题优先使用简短名，并限制在 20 字符以内；消息仅用于展示，不做非法字符清洗
- 页数与简介为空时，不在漫画信息中显示对应行
...
- 统一 JM 画质配置的代码兜底默认值为 75，与 `modules/jm/config.yaml` 保持一致
...
- **配置外置**：默认配置与界面字段定义从脚本移入各自模块的 `config.yaml`，改默认值不再需要动代码，也不会影响用户已保存的配置
  - JM 的下载项、默认值与 Pages 字段定义 → `modules/jm/config.yaml`
  - MiMo 与华数的模板变量说明 → 各自的 `config.yaml`
  - YAML 解析器升级为支持任意层级嵌套
- **JM 下载异常分类提示**：
  - 漫画不存在 → 提示检查 ID，若仅登录可见则填写 Cookie
  - 站点连接失败 → 提示稍后重试或配置代理
  - 部分图片失败 → 明确告知未生成不完整 PDF
- **JM PDF 画质可调**：新增 `jm_jpeg_quality`（默认 75），原图保持 webp 下载，生成 PDF 前再二次压缩为 JPEG
  - 实测 26 页漫画：质量 50→5.7 MB、75→7.7 MB、90→11.3 MB

### 📝 信息完善

- `info.json` 新增 `oname`（原始名）、`works`（作品系列）、`actors`（登场人物）、`page_count`（页数，以下载实际页数修正）、`description`（简介）

### ⚡ 变更

- JM 图片恢复为站点原图格式（webp），不再在下载阶段转码；压缩统一在生成 PDF 时进行
...
- **JM 下载 PDF 体积**：下载图片时即解码并转存为 JPEG，PDF 可直接嵌入而不再被无损转码
  - 实测同一部漫画（26 页）：PDF 由 **32.89 MB 降至 8.08 MB**，与图片总体积基本持平
  - 8 MB 已低于默认 10 MB 上限，不再出现「文件过大未发送」

### 🐛 修复

- JM 图片收集按文件名去重（同名的多格式文件只取最新一份），避免 PDF 出现重复页
...
- **JM 重复页**：图片收集只扫描当前约定的图片目录，不再把历史结构（`images/`、`pdf/`）中的旧图片重复计入 PDF
- **JM 目录大小写**：漫画根目录统一规范为大写 `JM<ID>`，历史遗留的小写目录会在下载时自动改名
- **JM 历史残留**：新结构图片就绪后自动清理同漫画下的 `images/`、`pdf/` 旧目录，避免重复占用磁盘
- **JM 历史缓存**：PDF 查找兼容 `<漫画根目录>/pdf/*.pdf` 旧结构
- **JM 文件名**：PDF 名与图片目录名保持一致，保留方括号等可读字符，非法字符替换为下划线
- **MiMo 账号卡片**：整张卡片均可点击打开配置页（此前只有内部区域可点）
- **MiMo 标签栏**：改为换行排列，不再出现横向滚动条
- **MiMo 密码字段**：账号密码与各类 Token 支持显示 / 隐藏切换
- **MiMo 设备字段**：设备标识与 User-Agent 可分别「恢复默认」，不再共用一次重置

### ♻️ 变更

- MiMo 默认 User-Agent 调整为 Android 客户端标识
- 华数模块移除「接口地址」配置项（接口地址固定于源码，配置无实际作用）
...
- JM 下载新增「返回漫画信息」开关，**默认关闭**：关闭后消息中只显示漫画 ID，避免不适宜的标题被发送到聊天引发风控

### ♻️ 界面重构（Pages）

- 模块页面改为标签式布局：「账号管理」「模板变量」「模块设置」各自独立成标签
- 账号编辑窗改为「账号配置」「消息模板」两个标签
- MiMo 账号配置按凭据方式分为三个标签：账号密码登录 / PassToken / ServiceToken
- 设备标识与 User-Agent 移入账号配置，未填写时保存自动填充插件默认值，并提供「恢复默认」按钮
- 华数查询页面结构与 MiMo 保持一致
...
- JM 下载目录统一为 `<数据目录>/JMDownload/JM<ID>/`：
  - 图片 → `JM<ID>/JM<ID>-漫画名/`
  - PDF → `JM<ID>/JM<ID>-漫画名.pdf`
  - 漫画信息 → `JM<ID>/info.json`
- MiMo 模块级「默认设备标识 / 默认 User-Agent」配置项移除，改为账号级配置
